...
import type { ResidenteSemSupervisorRow } from "@/components/views-sections"
...
export function EscalasSection() {
  const { data, loading, error, reload } = useApiList<ResidenteSemSupervisorRow>(
  ...
  function openReajuste(row: ResidenteSemSupervisorRow) {
  ...
  async function confirmReajuste(row: ResidenteSemSupervisorRow) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Lista as escalas de residentes sem supervisão de um preceptor doutor
          (vw_residentes_sem_supervisor) e permite mover o residente para outro dia/turno com{" "}
          <code>sp_reajustar_escala</code>. Se o destino já estiver ocupado, o banco recusa a
          troca e a API responde 409.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid gap-4">
        ...
        {!loading && !error && !data.length && (
          <p className="flex items-center gap-2 py-6 text-center text-sm text-muted-foreground">
            <ShieldAlert className="size-4" /> Nenhuma escala sem supervisor no momento.
          </p>
        )}
